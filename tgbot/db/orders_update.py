...
async def reg_author(id,name,card,speciality):
    base = psycopg2.connect(
        ...
        host=config.db.host,)
    cur = base.cursor()
    cur.execute("SELECT * FROM authors_ids WHERE authors_id = %s",(str(id),))
    author_id = cur.fetchone()
    print(author_id)
    data = (id,name,card,speciality,author_id[0])
    cur.execute('INSERT INTO authors (id, full_name, card,speciality,authors_ids,busyness,plane_busyness)  VALUES (%s,%s,%s,%s,%s,0,1)', data)
...
async def confirm_order(order_id, author_id):
    ...
    print(order_id)
    cur.execute('UPDATE orders SET author_id=%s, author_name = %s WHERE id=%s', data)
...
async def new_order(order_id):
    base = psycopg2.connect(
        ...
        host=config.db.host,)
    cur = base.cursor()
    print('cganged to - Знайти автора')
    data = ('Знайти автора',str(order_id))
    cur.execute('UPDATE orders SET status = %s WHERE sub_id=%s', data)
